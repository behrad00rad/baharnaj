...
function ServiceManagement() {
  ...
  return <div className="admin-page service-admin-page">
    ...
    {form && <div className="modal-backdrop" onMouseDown={() => setForm(null)}><form className="admin-modal service-editor" onSubmit={submit} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <span className="admin-kicker">{form.id ? "ویرایش خدمت" : "سرویس جدید"}</span><h2>{form.persian_name || "محتوای سرویس"}</h2>
      <fieldset><legend>اطلاعات اصلی</legend><div className="service-editor-grid">
        ...
        <label>نشانی صفحه خدمت<input dir="ltr" value={form.slug || ""} onChange={(event) => update("slug", event.target.value)} /><small>تغییر این مقدار، آدرس عمومی سرویس را تغییر می‌دهد.</small></label>
      </div></fieldset>
      <fieldset><legend>محتوای صفحه سرویس</legend><label>معرفی کوتاه<textarea maxLength="320" value={form.short_description || ""} onChange={(event) => update("short_description", event.target.value)} /></label><label>توضیحات کامل خدمت<textarea className="service-article-input" value={form.description || ""} onChange={(event) => update("description", event.target.value)} /></label></fieldset>
      ...
      <p>قابل رزرو بودن را فقط برای خدماتی روشن کنید که مشتری می‌تواند وقت آن‌ها را آنلاین بگیرد.</p>
...
function Finance() {
  ...
  return <div className="admin-page finance-page">
    ...
    <PanelDisclosure title="درآمد خالص، ارزش خدمات و کمیسیون"><div className="finance-stat-grid">
    ...
    <PanelDisclosure title="نمودار درآمد و سهم خدمات">
