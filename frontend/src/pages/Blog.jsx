  ...
  return <>
    ...
    <div className="blog-index container">
      {state !== "ready" ? <PublicState state={state} empty="هنوز مقاله‌ای منتشر نشده است." /> : !posts.length ? <PublicState state="ready" empty="در این دسته‌بندی هنوز مقاله‌ای منتشر نشده است." /> : <>
        {featured && <section className={`featured-post ${featured.cover_image_url ? "" : "featured-post-no-media"}`}>{featured.cover_image_url && <MediaImage src={featured.cover_image_url} alt={featured.cover_alt_text || featured.title} eager />}<div><p className="eyebrow">پیشنهاد مطالعه</p><span>{featured.category?.name || "مجله"}</span><h2><Link to={`/blog/${featured.slug}`}>{featured.title}</Link></h2><p>{featured.excerpt}</p><Link className="button" to={`/blog/${featured.slug}`}>مطالعه مقاله <span>←</span></Link></div></section>}
