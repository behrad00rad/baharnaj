...
export function BlogManagement() {
  ...
  return <div className="admin-page blog-admin-list">
    ...
    <div className="blog-advanced-filters"><select value={filters.category} onChange={(e) => setFilters({ ...filters, category: e.target.value })}><option value="">همه دسته‌ها</option>{options.categories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select value={filters.tag} onChange={(e) => setFilters({ ...filters, tag: e.target.value })}><option value="">همه برچسب‌ها</option>{options.tags.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select value={filters.author} onChange={(e) => setFilters({ ...filters, author: e.target.value })}><option value="">همه نویسندگان</option>{options.users.filter((item) => item.role === "admin").map((item) => <option key={item.id} value={item.id}>{[item.first_name,item.last_name].filter(Boolean).join(" ") || item.username}</option>)}</select><label>از تاریخ<input type="date" value={filters.date_from} onChange={(e) => setFilters({ ...filters, date_from: e.target.value })} /></label><label>تا تاریخ<input type="date" value={filters.date_to} onChange={(e) => setFilters({ ...filters, date_to: e.target.value })} /></label><button onClick={() => setFilters({ category: "", tag: "", author: "", date_from: "", date_to: "" })}>پاک‌کردن فیلترها</button></div>
...
export function BlogEditor() {
  ...
  return <div className="admin-page blog-editor-page">
    ...
    <div className="blog-editor-layout"><main className="blog-main-editor">
      ...
      <section><h3>نشانی مقاله</h3><label>Slug<input dir="ltr" value={post.slug} onChange={(e) => change("slug", e.target.value)} /></label>{post.status === "published" && <p className="setting-warning">تغییر این مقدار، نشانی عمومی مقاله را تغییر می‌دهد و ممکن است لینک‌های قبلی را بشکند.</p>}</section>
      ...
      <section><h3>SEO و شبکه‌های اجتماعی</h3><label>عنوان SEO<input maxLength="160" value={post.seo_title} onChange={(e) => change("seo_title", e.target.value)} /><small>{post.seo_title.length}/۱۶۰</small></label><label>توضیح SEO<textarea rows="4" maxLength="320" value={post.seo_description} onChange={(e) => change("seo_description", e.target.value)} /><small>{post.seo_description.length}/۳۲۰</small></label>{post.og_image_url && <img className="cover-preview" src={post.og_image_url} alt="پیش‌نمایش شبکه اجتماعی" />}<label>تصویر اختصاصی اشتراک‌گذاری<input type="file" accept="image/*" onChange={(e) => uploadField("og_image", e.target.files[0])} /></label></section>
    </aside></div>
  </div>;
}
