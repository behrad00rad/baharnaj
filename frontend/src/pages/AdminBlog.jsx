...
import { JalaliDatePicker, JalaliDateTimePicker } from "../components/DatePicker";
...
const faDate = (value) => formatJalaliDateTime(value, "—");
...
export function BlogManagement() {
  ...
  return <div className="admin-page blog-admin-list">
    ...
    <details className="blog-filter-disclosure"><summary>فیلترهای بیشتر</summary><div className="blog-advanced-filters"><select value={filters.category} onChange={(e) => setFilters({ ...filters, category: e.target.value })}><option value="">همه دسته‌ها</option>{options.categories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select value={filters.tag} onChange={(e) => setFilters({ ...filters, tag: e.target.value })}><option value="">همه برچسب‌ها</option>{options.tags.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select value={filters.author} onChange={(e) => setFilters({ ...filters, author: e.target.value })}><option value="">همه نویسندگان</option>{options.users.filter((item) => item.role === "admin").map((item) => <option key={item.id} value={item.id}>{[item.first_name,item.last_name].filter(Boolean).join(" ") || item.username}</option>)}</select><label>از تاریخ<JalaliDatePicker value={filters.date_from} onChange={(value) => setFilters({ ...filters, date_from: value })} minDate="" /></label><label>تا تاریخ<JalaliDatePicker value={filters.date_to} onChange={(value) => setFilters({ ...filters, date_to: value })} minDate="" /></label><button onClick={() => setFilters({ category: "", tag: "", author: "", date_from: "", date_to: "" })}>پاک‌کردن فیلترها</button></div></details>
    ...
    <PanelDisclosure title="مدیریت دسته‌بندی‌های وبلاگ"><div className="blog-category-manager"><div><span className="admin-kicker">ساختار مجله</span><h2>دسته‌بندی‌ها</h2><p>دسته‌های استفاده‌شده به‌جای حذف، غیرفعال می‌شوند تا ارتباط مقالات حفظ شود.</p></div><form onSubmit={(e) => { e.preventDefault(); manageCategory(null, "create"); }}><input placeholder="نام دسته‌بندی تازه" value={categoryName} onChange={(e) => setCategoryName(e.target.value)} /><button className="admin-secondary">ایجاد دسته</button></form><div>{options.categories.map((category) => <article key={category.id}><span><strong>{category.name}</strong><small>{new Intl.NumberFormat("fa-IR").format(category.post_count || 0)} مقاله · {category.is_active ? "فعال" : "غیرفعال"}</small></span><button onClick={() => manageCategory(category, "edit")}>ویرایش</button>{category.is_active && <button onClick={() => manageCategory(category, "archive")}>غیرفعال‌کردن</button>}</article>)}</div></div></PanelDisclosure>
  </div>;
}
...
export function BlogEditor() {
  ...
  const removeCategory = async (category) => {
    if (!window.confirm(`دسته «${category.name}» حذف شود؟ دسته‌های دارای مقاله غیرفعال می‌شوند و مقاله‌ای حذف نمی‌شود.`)) return;
    setBusy(true);
    try {
      await api.delete(`admin/blog/categories/${category.id}/`);
      await fetchOptions();
      setNotice("دسته‌بندی به‌روزرسانی شد.");
    } catch (error) {
      setNotice(firstError(error, "حذف دسته‌بندی انجام نشد."));
    } finally {
      setBusy(false);
    }
  };
  const removeTag = async (tag) => {
    if (!window.confirm(`برچسب «${tag.name}» حذف شود؟ مقاله‌ای حذف نمی‌شود و این برچسب از مقالات برداشته خواهد شد.`)) return;
    setBusy(true);
    try {
      await api.delete(`admin/blog/tags/${tag.id}/`);
      setPost((current) => ({ ...current, tags: (current.tags || []).filter((tagId) => Number(tagId) !== Number(tag.id)) }));
      setDirty(true);
      await fetchOptions();
      setNotice("برچسب حذف شد.");
    } catch (error) {
      setNotice(firstError(error, "حذف برچسب انجام نشد."));
    } finally {
      setBusy(false);
    }
  };
  ...
  return <div className="admin-page blog-editor-page">
    ...
    <div className="blog-editor-layout"><main className="blog-main-editor">
      ...
      <section><h3>انتشار</h3><span className={`blog-status ${post.status}`}>{statusLabels[post.status]}</span><label>زمان‌بندی انتشار (تقویم شمسی)<JalaliDateTimePicker value={post.scheduled_publish_at} onChange={(value) => change("scheduled_publish_at", value)} /></label><div className="publish-actions">{post.status !== "published" && <button type="button" className="admin-success" disabled={busy} onClick={() => statusAction("publish")}>انتشار اکنون</button>}<button type="button" disabled={busy || !post.scheduled_publish_at} onClick={() => statusAction("schedule")}>زمان‌بندی</button>{post.status === "published" && <button type="button" onClick={() => statusAction("unpublish")}>لغو انتشار</button>}<button type="button" onClick={() => statusAction("archive")}>آرشیو</button></div></section>
      ...
      <section><h3>دسته‌بندی</h3><select value={post.category || ""} onChange={(e) => change("category", e.target.value ? Number(e.target.value) : null)}><option value="">بدون دسته‌بندی</option>{options.categories.map((item) => <option value={item.id} key={item.id}>{item.name}{!item.is_active ? " (غیرفعال)" : ""}</option>)}</select><span className="admin-inline-option"><input aria-label="نام دسته‌بندی جدید وبلاگ" placeholder="نام دسته‌بندی جدید" value={newCategory} onChange={(e) => setNewCategory(e.target.value)} /><button type="button" disabled={busy} onClick={() => createOption("categories", newCategory)}>+ افزودن دسته‌بندی جدید</button></span><details className="admin-category-manager"><summary><span>مدیریت دسته‌بندی‌های وبلاگ</span><small>{options.categories.filter((item) => item.is_active).length} دسته‌بندی</small></summary>{options.categories.filter((item) => item.is_active).length ? <div className="admin-option-list">{options.categories.filter((item) => item.is_active).map((category) => <span key={category.id}>{category.name}<button type="button" className="admin-option-delete" disabled={busy} onClick={() => removeCategory(category)} aria-label={`حذف دسته‌بندی ${category.name}`}>×</button></span>)}</div> : <p className="admin-category-empty">هنوز دسته‌بندی فعالی ساخته نشده است.</p>}<p className="admin-category-note">دسته‌های دارای مقاله غیرفعال می‌شوند و مقاله‌ای حذف نمی‌شود.</p></details></section>
      <section><h3>برچسب‌ها</h3><div className="checkbox-grid">{options.tags.map((tag) => <label key={tag.id}><input type="checkbox" checked={(post.tags || []).includes(tag.id)} onChange={(e) => change("tags", e.target.checked ? [...post.tags, tag.id] : post.tags.filter((id) => id !== tag.id))} />{tag.name}</label>)}</div><span className="admin-inline-option"><input aria-label="نام برچسب جدید وبلاگ" placeholder="نام برچسب جدید" value={newTag} onChange={(e) => setNewTag(e.target.value)} /><button type="button" disabled={busy} onClick={() => createOption("tags", newTag)}>+ افزودن برچسب جدید</button></span><details className="admin-category-manager"><summary><span>مدیریت برچسب‌ها</span><small>{options.tags.length} برچسب</small></summary>{options.tags.length ? <div className="admin-option-list">{options.tags.map((tag) => <span key={tag.id}>{tag.name}<button type="button" className="admin-option-delete" disabled={busy} onClick={() => removeTag(tag)} aria-label={`حذف برچسب ${tag.name}`}>×</button></span>)}</div> : <p className="admin-category-empty">هنوز برچسبی ساخته نشده است.</p>}<p className="admin-category-note">حذف برچسب، مقاله‌ها را حذف نمی‌کند.</p></details></section>
