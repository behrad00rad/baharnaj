...
export function BlogManagement() {
  ...
  return <div className="admin-page blog-admin-list">
    ...
    <div className="blog-advanced-filters"><select value={filters.category} onChange={(e) => setFilters({ ...filters, category: e.target.value })}><option value="">همه دسته‌ها</option>{options.categories.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select value={filters.tag} onChange={(e) => setFilters({ ...filters, tag: e.target.value })}><option value="">همه برچسب‌ها</option>{options.tags.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select value={filters.author} onChange={(e) => setFilters({ ...filters, author: e.target.value })}><option value="">همه نویسندگان</option>{options.users.filter((item) => item.role === "admin").map((item) => <option key={item.id} value={item.id}>{[item.first_name,item.last_name].filter(Boolean).join(" ") || item.username}</option>)}</select><label>از تاریخ<JalaliDatePicker value={filters.date_from} onChange={(value) => setFilters({ ...filters, date_from: value })} minDate="" /></label><label>تا تاریخ<JalaliDatePicker value={filters.date_to} onChange={(value) => setFilters({ ...filters, date_to: value })} minDate="" /></label><button onClick={() => setFilters({ category: "", tag: "", author: "", date_from: "", date_to: "" })}>پاک‌کردن فیلترها</button></div>
