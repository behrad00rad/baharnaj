...
export default function EmployeeLayout() {
  ...
  const logout = async () => { await disableCurrentPushSubscription(); clearSession(); navigate('/login', { replace: true }) }
  return <div className="employee-app" dir="rtl"><header className="employee-header"><div className="employee-brand"><span>ب</span><div><b>بهارناژ</b><small>روز کاری من</small></div></div><button type="button" onClick={() => navigate('/')}>بازگشت به سایت</button><NotificationBell /></header><main><Outlet /></main><nav className="employee-nav">{links.map(([path, label, icon]) => <NavLink end={path === '/'} key={path} to={`/employee${path}`}><i>{icon}</i><span>{label}</span></NavLink>)}<button onClick={logout}><i>↪</i><span>خروج</span></button></nav></div>
}
