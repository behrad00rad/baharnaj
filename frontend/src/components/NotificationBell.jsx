...
import { listenForForegroundMessages, registerFirebaseDevice } from '../shared/firebase'
...
function PushPermissionButton({ onNotificationSent }) {
  ...
  const test = async () => {
    ...
    try {
      ...
      setMessage('اعلان آزمایشی ارسال شد؛ زنگ اعلان و اعلان سیستم را بررسی کنید.')
      onNotificationSent()
    } catch {
  ...
  return <div className="push-setting"><span>اعلان‌های مرورگر: {state === 'enabled' ? 'فعال' : 'غیرفعال'}</span><div className="push-actions"><button type="button" onClick={state === 'enabled' ? disable : enable} disabled={state === 'denied'}>{state === 'enabled' ? 'غیرفعال‌سازی' : 'فعال‌سازی اعلان‌ها'}</button>{state === 'enabled' && <button type="button" onClick={test}>ارسال آزمایشی</button>}</div>{statusMessage && <small className="push-message">{statusMessage}</small>}</div>
}
...
export default function NotificationBell() {
  ...
  useEffect(() => {
    let active = true
    let unsubscribe = () => {}
    listenForForegroundMessages(() => { refreshCount(); if (open) refreshList() }).then((cleanup) => {
      if (!active) {
        cleanup()
        return
      }
  ...
  const notificationSent = () => { refreshList(); refreshCount() }
  return <div className="notification-root" ref={root}><button className="notification-bell" type="button" title="اعلان‌ها" aria-label={`اعلان‌ها، ${unread} خوانده‌نشده`} aria-expanded={open} onClick={toggle}>◔{unread > 0 && <b>{unread > 99 ? '۹۹+' : unread.toLocaleString('fa-IR')}</b>}</button>{open && <section className="notification-panel"><header><strong>اعلان‌ها</strong>{unread > 0 && <button type="button" onClick={readAll}>خواندن همه</button>}</header><div className="notification-list">{loading && <p>در حال دریافت…</p>}{error && <p className="notification-error">{error}</p>}{!loading && !error && !notifications.length && <p>اعلان تازه‌ای ندارید.</p>}{notifications.map((item) => <button type="button" className={item.is_read ? '' : 'unread'} onClick={() => select(item)} key={item.id}><strong>{item.title}</strong><span>{item.message}</span><time>{date(item.created_at)}</time></button>)}</div><PushPermissionButton onNotificationSent={notificationSent} /></section>}</div>
}
